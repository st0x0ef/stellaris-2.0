...
public class StellarisTreeFeature<FC extends StellarisTreeConfiguration> extends TreeFeature {

    protected StellarisTreeFeature(Codec<FC> codec)
    {
        super((Codec)codec);
    }
    ...
    protected boolean canReplace(LevelAccessor level, BlockPos pos)
    {
        return TreeFeature.isAirOrLeaves(level, pos) || level.isStateAtPosition(pos, (state) -> {
            ...
            // Might need to add our own tag check for vines
            return state.is(BlockTags.REPLACEABLE_BY_TREES) || state.is(BlockTags.SAPLINGS) || block == Blocks.VINE || state.is(BlockTags.CAVE_VINES) || block instanceof VegetationBlock;
        });
    }
