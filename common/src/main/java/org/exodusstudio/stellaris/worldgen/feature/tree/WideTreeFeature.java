package org.exodusstudio.stellaris.worldgen.feature.tree;
...
import org.exodusstudio.stellaris.util.biome.GeneratorUtil;
import org.exodusstudio.stellaris.worldgen.feature.configurations.SpreadTreeConfiguration;

import java.util.function.BiConsumer;

public class WideTreeFeature extends StellarisTreeFeature<SpreadTreeConfiguration> {
    public Block supportingBlock = Blocks.DIRT;

    public WideTreeFeature(Codec<SpreadTreeConfiguration> codec) {
        super(codec);
    }

    public WideTreeFeature(Codec<SpreadTreeConfiguration> codec, Block supportingBlock) {
        super(codec);
        this.supportingBlock = supportingBlock;
    }

    public boolean checkSpace(LevelAccessor world, BlockPos pos, int baseHeight, int height, SpreadTreeConfiguration config)
    {
    ...
    public void generateLeafLayer(LevelAccessor world, RandomSource rand, BlockPos pos, int leavesRadius, int trunkStart, int trunkEnd, FoliagePlacer.FoliageSetter leaves, SpreadTreeConfiguration config)
    {
    ...
    public void generateBranch(LevelAccessor world, RandomSource rand, BlockPos pos, Direction direction, int length, BiConsumer<BlockPos, BlockState> logs, FoliagePlacer.FoliageSetter leaves, SpreadTreeConfiguration config)
    {
    ...
    protected boolean doPlace(WorldGenLevel world, RandomSource random, BlockPos startPos, BiConsumer<BlockPos, BlockState> roots, BiConsumer<BlockPos, BlockState> logs, FoliagePlacer.FoliageSetter leaves, SpreadTreeConfiguration config)
    {
                ...
                if (didPlace) {
                    ...
                    for (int y = 1; y < 4; y++)
                    {
                        ...
                        if (state.isAir() || state.is(BlockTags.REPLACEABLE_BY_TREES) || isDirt(state)) {
                            world.setBlock(local, supportingBlock.defaultBlockState(), 3);
                        }
                    }
                }
            }
    ...
    protected boolean generateBush(BiConsumer<BlockPos, BlockState> logs, FoliagePlacer.FoliageSetter leaves, LevelAccessor world, RandomSource random, BlockPos pos, SpreadTreeConfiguration config)
    {
        int height = 2;
