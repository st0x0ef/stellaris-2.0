...
public record FlagItemRenderer(Identifier texture, FlagBlockModel baseModel) implements SpecialModelRenderer<Identifier> {
    ...
    @Override
    public void submit(Identifier texture, PoseStack poseStack, SubmitNodeCollector nodeCollector, int packedLight, int packedOverlay, boolean hasFoil, int outlineColor) {
        poseStack.pushPose();

        poseStack.mulPose(Axis.ZP.rotationDegrees(180));
        poseStack.translate(-0.5D, -1.5, 0.5D);

        nodeCollector.submitModelPart(baseModel.root(), poseStack, RenderTypes.entityCutoutCull(texture), packedLight, packedOverlay, null);
